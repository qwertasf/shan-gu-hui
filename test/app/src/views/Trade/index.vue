<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressList" :key="address.id" @click="changeAdress(address)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="shop in tradeList.detailArrayList" :key="shop.skuId">
          <li>
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        ...
        orderDetailList: this.tradeList.detailArrayList //购物车商品信息
      }

      //发请求:提交订单
      try {
        await this.$store.dispatch('getOrder', { tradeNo, data })
        //将来提交订单成功【订单ID生成】，路由跳转pay页面，进行支付
        // console.log(this.tradeNO)
        this.$router.push({ path: '/pay', query: { orderId: this.tradeNO } })
      } catch (error) {
        alert(error.message)
      }
    }
