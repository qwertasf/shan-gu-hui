          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index) in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{active:item1.isChecked==1}" v-for="item1 in item.spuSaleAttrValueList" :key="item1.id" @click="changeChecked(item1,item.spuSaleAttrValueList)">{{ item1.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuValue" @change="checked">
                <a href="javascript:" class="plus" @click="skuValue++">+</a>
                <a href="javascript:" class="mins" @click="skuValue > 1 ? skuValue-- : 1">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    changeChecked(object, spuSaleAttrValueList) {
    ...
    checked(e) {
      ...
      if (isNaN(value) || value < 1) this.skuValue = 1
      else this.skuValue = parseInt(value)
    },
    addShopCar() {
      this.$store.dispatch('getCartSuccess', {
        skuId: this.$route.params.skuid,
        skuNum: this.skuValue
      })
      sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
      this.$router.push({
        path: '/addToCart',
        query: { skuNum: this.skuValue }
      })
    }
  }
}
