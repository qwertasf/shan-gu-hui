<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="(order,index) in myorderlist.records" :key="index">
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(item,index) in order.orderDetailList" :key="item.id">
              ...
              <td :rowspan="order.orderDetailList.length" v-show="index==0" width="13%" class="center">
                <ul class="unstyled">
                  <li>总金额¥{{ item.splitTotalAmount }}.00</li>
                  <li>在线支付</li>
      ...
      <Pagination :pageNo="page" :pageSize="limit" :total="total" :pageCount=5 @nextPage="nextpage"></Pagination>
    </div>
    <!--猜你喜欢-->
    <div class="like">
      <h4 class="kt">猜你喜欢</h4>
...
export default {
  ...
  computed: {
    ...mapState({
      myorderlist: state => state.trade.myorder
    })
  },
  methods: {
    async getdata() {
      const { page, limit } = this
      await this.$store.dispatch('getmyOrder', { page, limit })
      this.total = this.myorderlist.total
